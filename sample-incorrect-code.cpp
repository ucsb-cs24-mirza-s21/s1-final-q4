...
class smartQueue {
    public:
       // smartQueue(int mn, int mx) : min(mn), max(mx) {}
        void push(int key);
        void pop();
        int front();
        bool empty();
...
void smartQueue::push(int key) { // unfortunately not O(1)
    q.push(key);

    while (!min_.empty() && key < min_.front()) {
        min_.pop_front();
    }
    while (!min_.empty() && key < min_.back()) {
        min_.pop_back();
    }
    min_.push_back(key);

    while (!max_.empty() && key > max_.front()) {
        max_.pop_front();
    }
    while (!max_.empty() && key > max_.back()) {
        max_.pop_back();
    }
    max_.push_back(key);
}
...
void smartQueue::pop() {
    if (min_.front() == q.front()) min_.pop_front();
    if (max_.front() == q.front()) min_.pop_front();
    q.pop();
}
